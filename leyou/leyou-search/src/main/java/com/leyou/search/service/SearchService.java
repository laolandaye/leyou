...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        // 查询分类名称
        List<String> names = this.categoryClient.queryNamesByIds(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3())).getBody();
        ...
        // 查询出所有的搜索规格参数
        List<SpecParam> params = this.specificationClient.queryParams(null, spu.getCid3(), null, true).getBody();
        // 查询spuDetail。获取规格参数值
...
//        MatchQueryBuilder basicQuery = QueryBuilders.matchQuery("all", request.getKey()).operator(Operator.AND);
    ...
    private List<Map<String,Object>> getParamAggResult(Long id, QueryBuilder basicQuery) {
        ...
        // 查询要聚合的规格参数
        List<SpecParam> params = this.specificationClient.queryParams(null, id, null, true).getBody();
        // 添加聚合
            params.forEach(param -> {
    ...
    private List<Map<String,Object>> getCategoryAggResult(Aggregation aggregation) {
        ...
        List<String> names = this.categoryClient.queryNamesByIds(cids).getBody();
